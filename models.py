...
class SKIP_GRAM(nn.Module):
    ...
    def forward(self, x):
        """returns a list of all predicted context word's embedding"""
        context_out = []
        for i in range(self.context_len):
            x_ = self.embedding(x)
            context_word_i = self.fc1(x_)
            context_out.append(context_word_i)
        print ("shape was -->",torch.stack(context_out).shape, "there is an extra dimmension here")
        return torch.stack(context_out)